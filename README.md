...
File server is built as a stand-alone UDP-based server. It wait
for a message and then process the message as need be, replying to the given client.
...
contains the on-disk representation of your data structures; Use these system calls to access it: `open(), read(), write(),
lseek(), close(), fsync().`
